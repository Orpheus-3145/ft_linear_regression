...
class Plotter:
	...
	def set_current_ax(self, row: np.int64, col: np.int64) -> None:
		try:
			self.current_ax = self.axes[row, col]
		except IndexError:
			return
	
	def draw_fit_line(self, x_data: np.ndarray, y_data: np.ndarray, estimator, index_graph: tuple[np.int64, np.int64] = [0, 0], title: str = "", show_legend=False, **plot_args) -> None:
	...
	def draw_rmse_line(self, lr_values: np.ndarray, rmse_values: np.ndarray, index_graph: tuple[np.int64, np.int64] = [0, 0], is_logarithmic: bool = False) -> None:
		self.set_current_ax(index_graph[0], index_graph[1])
	...
	def draw_rsquared_line(self, lr_values: np.ndarray, r_squared_values: np.ndarray, index_graph: tuple[np.int64, np.int64] = [0, 0], is_logarithmic: bool = False) -> None:
		self.set_current_ax(index_graph[0], index_graph[1])
	...
	def add_info(self, pos_x: np.int64, pos_y: np.int64, info: str) -> None:
