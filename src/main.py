from ft_linear_regression.train import train_model_parallel, load_csv, estimate_y, TrainingData
from ft_linear_regression.plot import draw_scatter, show_data, plot_line, init_graph, draw_bars, plot_line
import numpy as np
...
def show_results(trained_data: list[TrainingData], x_data: np.ndarray) -> None:
	...
	for item in trained_data:
		print(item)
		plot_line(x_data, estimate_y(item.tetha_1, item.tetha_0, x_data), color=item.color, label=f"learning rate: {item.learning_rate:.8f}")
